...
export class WecomWebhook {
  ...
  async handleMessage(query, body) {
    ...
    if (msgtype === "text") {
      ...
      const mediaId = data.image?.media_id;
      const msgId = data.msgid || `msg_${Date.now()}`;
      const fromUser = data.from?.userid || "";
      const responseUrl = data.response_url || "";
      const chatType = data.chattype || "single";
      ...
      // 混合消息（图文混合）
      // https://developer.work.weixin.qq.com/document/path/101039#混合消息
      const msgId = data.msgid || `msg_${Date.now()}`;
      const fromUser = data.from?.userid || "";
      const responseUrl = data.response_url || "";
      const chatType = data.chattype || "single";
      ...
      // 解析混合消息的各个部分
      const mixedContent = data.mixed?.msg_item || [];
      const parsedItems = [];
      let textContent = "";
      ...
      for (const item of mixedContent) {
        if (item.type === "text") {
          ...
          parsedItems.push({
            type: "image",
            url: item.image?.url || "",
            mediaId: item.image?.media_id || "",
          });
        } else if (item.type === "voice") {
          parsedItems.push({
            type: "voice",
            mediaId: item.voice?.media_id || "",
          });
        } else if (item.type === "video") {
          parsedItems.push({
            type: "video",
            mediaId: item.video?.media_id || "",
          });
      ...
      // 文件消息
      const mediaId = data.file?.media_id;
      ...
      // 视频消息
      const mediaId = data.video?.media_id;
      const msgId = data.msgid || `msg_${Date.now()}`;
      const fromUser = data.from?.userid || "";
      const responseUrl = data.response_url || "";
      const chatType = data.chattype || "single";
      ...
      // 检查重复
      if (this.deduplicator.isDuplicate(msgId)) {
        logger.debug("Duplicate video message ignored", { msgId });
        return null;
      }
